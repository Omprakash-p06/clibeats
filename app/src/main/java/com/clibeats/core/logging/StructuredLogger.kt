        ...
        val json =
            buildString {
                ...
                when (event) {
                    ...
                    is StructuredEvent.SearchResponse ->
                        append(
                            "\"count\":",
                        ).append(event.count).append(",\"durationMs\":").append(event.durationMs)
                    is StructuredEvent.TrackSelected ->
                        append(
                            "\"trackId\":\"",
                        ).append(event.trackId).append("\",\"title\":\"").append(event.title.replace("\"", "\\\"")).append("\"")
                    is StructuredEvent.StreamRequest -> append("\"trackId\":\"").append(event.trackId).append("\"")
                    is StructuredEvent.StreamResolved ->
                        append(
                            "\"trackId\":\"",
                        ).append(event.trackId).append("\",\"durationMs\":").append(event.durationMs)
                    ...
                    is StructuredEvent.PlayerError ->
                        append(
                            "\"stage\":\"",
                        ).append(event.stage).append("\",\"error\":\"").append(event.error).append("\"")
                    is StructuredEvent.NetworkError ->
                        append(
                            "\"endpoint\":\"",
                        ).append(event.endpoint).append("\",\"message\":\"").append(event.message).append("\"")
                    is StructuredEvent.GatewayError ->
                        append(
                            "\"code\":\"",
                        ).append(event.code).append("\",\"message\":\"").append(event.message).append("\"")
                }
                append("}")
            }
        runCatching {
